	...
	if (airs < 25)
		...
		else
		{
			if (exploringWood < 10)
			{
				...
				xhttp.send();
				setTimeout(function(){$("#displaychangingtext").append("I cleave my path through a drooping canopy of thick vines, and a fantastical sight awaited - plump, red roses, moths flitting about, partaking in the Wood's gifts, before yielding so eagerly in the candle-flames that illuminate the scene like a tapestry. I flit among the moths for a while, learning their secrets to travelling the Wood, before settling upon having a drop of the rich, red nectar of the flowers. A singular plump, red drop -"); $("#displaychangingtext2").append("Through a red and viscous haze, I awake, sprawled in the grass and roots of the clearing, yet again. I do not remember what transpired, yet I remember more of the moth-secrets of travelling the Wood.");}, 200);
			}
			else
			{
				xhttp.onload = function() {document.getElementById("storylet").innerHTML = this.responseText;};
...
function loadWoodPonderOutcome()
{
	...
	xhttp.onload = function() {document.getElementById("storylet").innerHTML = this.responseText;};
			...
			xhttp.send();
			setTimeout(function(){$("#displaychangingtext").append("I struggle for hours on end trudging through the ways of mind and memory - to see anything resembling what a potential path may be through my experiences through the Wood. I take attention to every miniscule detail of the Wood - yet it yields only little, tiny scraps of knowledge - this is all I've learnt for now, until inspiration strikes.");}, 200);
		}
		else
		{
	xhttp.onload = function() {document.getElementById("storylet").innerHTML = this.responseText;};
